...
#define map_debug(idx, val)
    ...
    // keep the legacy direct paths that were verified with on-device. Gate
    // on the kernel version, NOT on va1_bits: 4.19/5.4 both run 39-bit VAs.
    // The stored version is the raw setup_preset kernel version bytes:
    // [_ pad][patch][minor][major] little-endian.
